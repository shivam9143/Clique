package yoda.huddl.live.huddlgallery.adapter
...
import yoda.huddl.live.huddlgallery.R
...
class GalleryPicturesAdapter(private val list: List<GalleryPicture>) : RecyclerView.Adapter<GVH>() {
    ...
    private lateinit var afterSelectionCompleted: (index : Int, isSelected : Boolean) -> Unit
    ...
    fun setAfterSelectionListener(afterSelectionCompleted: (Int, Boolean) -> Unit) {
...
//                afterSelectionCompleted(position)
            } else
                onClick(picture)
        }
...
//            afterSelectionCompleted(position)
    ...
    private fun handleSelection(position: Int, context: Context) {
        ...
        var isSelected = if (picture.isSelected) {
            false
        } else {
    ...
    fun getSelectedItemsLiveData() = liveData<ArrayList<Int>> {
        selectedIndexList.map {
            list[it]
        }
    }
